...
namespace CMS.Forms.Faculty
{
    public partial class UpdateFaculty : System.Web.UI.Page
    {
          ...
          // Bind_ddlCountryPresent();
            try
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    objEWA.EmployeeId = Convert.ToInt32(Session["UserCode"]);
                    BindDDLdata();
        ...
        {
            cn.Open();
            SqlCommand cmd = new SqlCommand("select * from stateCity where StateId ='" + ddlPresentStATE.SelectedValue + "'", cn);

            SqlDataReader dr = cmd.ExecuteReader();
            ddlPresentCity.DataSource = dr;
            ddlPresentCity.Items.Clear();
            //ddlPresentCity.Items.Add("--Please Select city--");
            ddlPresentCity.DataTextField = "City";
        ...
        {
            cn.Open();
            SqlCommand cmd = new SqlCommand("select * from stateCity ", cn);
