...
{
    public partial class PayInstallmentFees : System.Web.UI.Page
    {
        //Objects
        #region[Objects]
        ...
        private void BindFeesPaidDetailsGrid()
        {
            try
            {
                ...
                objEWA.AcadmicYearId =Convert.ToInt32(DDLAcademicYear.SelectedValue);
                //
        ...
        private void BindFeesPendingDetailsGrid()
        {
            try
            {
                ...
                //if(txtStudentNameId.Text=="")
                //{
                //    strValue= txtStudFilter.Text;
                //}
                //else
                //{
                    strValue = txtStudentNameId.Text;
               // }
                lblStudentCode.Text = strValue;
            ...
            try
            {
                ...
                foreach (GridViewRow row in GrdFeesDetails.Rows)
                {
                    ...
                    if (Particular.Checked)
                    {
                        ...
                        //Added by Ashwini 1-oct-2020
                        TextBox txtPayAmt1 = row.FindControl("txtPayamount") as TextBox;
                        Label lblPendingAmt = row.FindControl("lblTotalPendingAmount") as Label;
                        Label lblPaidAmt = row.FindControl("lblPaidAmount") as Label;
                        Label lblTotalAmt = row.FindControl("lblTotalAmount") as Label;
                        string PayAmt = txtPayAmt1.Text;
                        string PendingAmt = lblPendingAmt.Text;
                        double dblPendingAmt = PendingAmt != "" ? Convert.ToDouble(PendingAmt) : 0;
                        double PayAmount = PayAmt!=""? Convert.ToDouble(PayAmt):0;
                        double PaidAmount = lblPaidAmt.Text != "" ? Convert.ToDouble(lblPaidAmt.Text) : 0;
                        double TotalAmount = lblTotalAmt.Text != "" ? Convert.ToDouble(lblTotalAmt.Text) : 0;
                        double FinalPaid = TotalAmount - PaidAmount;
                        if (PayAmount<=0)
                        {
                            ShwMsg.Visible = true;
                    ...
                    if (txtFlag == 0)
                {
                    ...
                    foreach (GridViewRow row in GrdFeesDetails.Rows)
                    {
                        ...
                        if (row != null)
                        {
                         ...
                         //   var todaysdate = DateTime.Today.Date.ToString("dd/MM/yyyy");
                            ...
                            lblDate.Text = Convert.ToDateTime(txtReceiptDate.Text).ToString("dd/MM/yyyy");  //todaysdate;
                    ...
                    //            //string query = "select Particular,'First/Second Term' Term,convert(decimal(10,2), Amount) DueFees,convert(decimal(10,2), sum(PaidAmount)) ReceivedFee," +
                    //            //    "convert(decimal(10,2), (Amount-(select sum(tt.PaidAmount)  from tblstudentfeespaid tt where tt.feesdetailsid=tblstudentfeespaid.feesdetailsid  " +
                    //            //    "and  tt.StudentId=tblstudentfeespaid.StudentId))) BalanceFee from tblstudentfeespaid inner join tblfeesdetails " +
                    //            //    "on tblstudentfeespaid.feesdetailsid = tblfeesdetails.feesdetailsid where studentid = '"+ StudentId1 + "' and GroupReceiptNo="+ GroupReceiptNo + "" +
                    //            //    "group by  particular,Amount,tblstudentfeespaid.feesdetailsid,tblstudentfeespaid.StudentId order by tblstudentfeespaid.feesdetailsid";
                    ...
                    //            //            "and FeesDetailsId=tblStudentFeesPaid.FeesDetailsId )'Total Fees',sum(paidamount) 'Received Fees'," +
                    //            //            "(select amount from tblfeesdetails where feesid=tblstudentfeespaid.FeesId and FeesDetailsId=tblStudentFeesPaid.FeesDetailsId )-sum(paidamount) 'Due Fees' " +
                    //            //            "from tblstudentfeespaid " +
                    //            //            "where studentid='" + StudentId1 + "' and FeesId=" + ViewState["FeesId"].ToString() + " and GroupReceiptNo=" + GroupReceiptNo + " " +
                    //            //            "group by remark,FeesId,FeesDetailsId"+
                    //            //            "  union all " +
                    //            //"select remark Particular,'First/Second Term' Term, TotalAmount 'Total Fees',sum(paidamount) 'Received Fees',TotalAmount-sum(paidamount) 'Due Fees' from tblStudentBusFeesPaid" +
                    //            //" where studentid='" + StudentId1 + "'and ReceiptNo=" + GroupReceiptNo + " group by remark,TotalAmount";
                    //            //"Select Particular,'First/Second Term' Term,convert(decimal(10,2), Amount) 'Due Fee', (select isnull(sum(paidamount),0) " +
                    //            //"from tblstudentfeespaid where studentid='" + StudentId1 + "' and feesdetailsid=tblfeesdetails.Feesdetailsid)'Received Fee'," +
                    //            //"Amount-(select isnull(sum(paidamount),0) from tblstudentfeespaid where studentid='" + StudentId1 + "' " +
                    //            //"and feesdetailsid=tblfeesdetails.Feesdetailsid)'Balance Fee'from tblfeesdetails where feesid=" + ViewState["FeesId"].ToString() +"";
        ...
        protected void btnGenerateReceipt_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Response.Redirect("~/Forms/Reports/PrintFeesReciept1.aspx?Code=" + StudCode);
            }
            catch (Exception exp)
            {
                GeneralErr(exp.Message.ToString());
        ...
        public void BindStudentName()
        {
            ...
            cmd.CommandText = "sp_getStudentForPayFeesInstallment";
            cmd.Connection = cn;
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                ddlStudentname.DataValueField = "usercode";
