    ...
    public partial class AddStaff : System.Web.UI.Page
    {
        ...
        private void BindDDLdata()
        {
            try
            {
                ...
                cn.Open();
                SqlCommand cmd = new SqlCommand("SELECT CasteCategoryId,CasteCategoryName FROM tblCasteCategory where OrgId='" + Convert.ToInt32(Session["OrgID"]) + "'", cn);
                SqlDataReader dr = cmd.ExecuteReader();
...
string MType)
        ...
        protected void btnUploadPhoto_Click(object sender, EventArgs e)
        {
            try
            {
                    ...
                    if (matchExtension.Contains(fileExtension) && matchMimeType.Contains(fileMimeType))
                    {
                        if (fileLengthInKB < 1024)//&& fileLengthInKB > 4
                        {
                            if (PhotoUpload.HasFile)
                            {
                                ...
                                ViewState["FacultyPhoto"] = imgPhotobyte;
                                string base64String = Convert.ToBase64String(imgPhotobyte, 0, imgPhotobyte.Length);
                                img_FacultyImage.ImageUrl = "data:image/png;base64," + base64String;
                                img_FacultyImage.Visible = true;
                                txtimg.Text = "1";
                            }
                            //objEWA.Photo = (byte[])ViewState["FacultyPhoto"];
                        }
                ...
                {
                    msgBox.ShowMessage("Please choose a file !!!", "Information", UserControls.MessageBox.MessageStyle.Information);
